			...
			struct hostapd_frame_info *fi)
{
	...
	/* Get the sta_info struct for this station/AP */
			   ...
			   size_t len, int ok)
{
	...
	wpa_printf(MSG_INFO, "WGTT: %s %s():%d Params: auth_alg:%d, auth_transaction:%d, status_code:%d\n", __FILE__, __FUNCTION__, __LINE__, auth_alg, auth_transaction, status_code);
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	if ((!hapd->conf->ieee802_1x && !hapd->conf->wpa && !hapd->conf->osen) ||
		...
		wpa_printf(MSG_INFO, "WGTT: WORKING %s:%d Set to Authorized. \n", __FUNCTION__, __LINE__);
		wpa_printf(MSG_INFO, "WGTT: WORKING: Auth_alg in hostapd: %x", sta->auth_alg);
	}
	...
	wpa_printf(MSG_INFO, "WGTT: WORKING: Flags in hostapd: %x", sta->flags);
	wpa_printf(MSG_INFO, "WGTT: WORKING: Auth_alg in hostapd: %x", sta->auth_alg);
	wpa_printf(MSG_INFO, "WGTT: WORKING: status in hostapd: %d", status);
