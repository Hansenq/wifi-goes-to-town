...
int hostapd_set_sta_flags(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (((!hapd->conf->ieee802_1x && !hapd->conf->wpa) ||
		...
		wpa_printf(MSG_INFO, "WGTT: %s:%d Flags set to authorized.\n", __FUNCTION__, __LINE__);
	}
	wpa_printf(MSG_INFO, "WGTT: %s:%d total_flags:0x%x set_flags:0x%x.\n", __FUNCTION__, __LINE__, total_flags, set_flags);
